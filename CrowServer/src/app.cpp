...
int main(int argc, char** argv){
    ...
    server.route("/upper", [](const request& req, response& res){
    if (req.url_params.hasKey("word")){
        std::string word = req.url_params.get("word");

        std::string answer = upperWord(word);
    ...
    server.route("/lower", [](const request& req, response& res){
    if (req.url_params.hasKey("word")){
        std::string word = req.url_params.get("word");

        std::string answer = lowerWord(word);
